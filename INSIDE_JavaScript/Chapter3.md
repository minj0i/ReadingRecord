...
```
### **3.5.8 Array() 생성자 함수**
생성자 함수로 배열과 같은 객체를 생성할 때는 반드시 new 연산자를 같이 써야 함
- 호출할 때 인재가 1개이고, 숫자일 경우: 호출된 인자를 length로 갖는 빈 배열 생성
- 그 외의 경위 호출된 인자를 요소로 갖는 매열 생성

### **3.5.9 유사 배열 객체**
length 프로퍼티를 가진 객체를 유사 배열 객체(array-like objects)라고 부른다.   
`apply() 메서드`를 사용하면 객체지만 표준 배열 메서드를 활용하는 것이 가능함
```javascript
var arr = ['bar'];
